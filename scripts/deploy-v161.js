...
async function main() {
  ...
  for (const f of ['Centrio Setup 1.6.1.exe', 'Centrio Setup 1.6.1.exe.blockmap', 'latest.yml']) {
    await upload(path.join(DIST, f), `${DL}/${f}`, f);
  }
  console.log('\n🔍 Проверка...');
  console.log(await run('curl -sI "https://download.centrio.me/Centrio%20Setup%201.6.1.exe" | head -2'));
  console.log('✅ v1.6.1 задеплоен!');
  await sftp.end();
}

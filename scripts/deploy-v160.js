...
async function main() {
  ...
  for (const f of ['Centrio Setup 1.6.0.exe', 'Centrio Setup 1.6.0.exe.blockmap', 'latest.yml']) {
    await upload(path.join(DIST, f), `${DL}/${f}`, f);
  }

  console.log('\n🔍 Проверка...');
  console.log(await run('curl -sI "https://download.centrio.me/Centrio%20Setup%201.6.0.exe" | head -2'));
